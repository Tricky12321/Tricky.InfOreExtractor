...
public class InfExtractorMachineWindow : BaseMachineWindow
{
	...
	public override bool ButtonClicked(string name, SegmentEntity targetEntity)
	{
		...
		if (name.Equals("drill_motor"))
		{
			if (infOreExtractor.AttemptUpgradeDrillMotor(WorldScript.mLocalPlayer))
			{
				...
				if (!WorldScript.mbIsServer)
				{
					...
					NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "UpgradeMotor", null, motor, infOreExtractor, 0f);
				}
				return true;
			}
		}
		else if (name.Equals("cutter_head"))
		{
			if (infOreExtractor.AttemptUpgradeCutterHead(WorldScript.mLocalPlayer))
			{
				...
				if (!WorldScript.mbIsServer)
				{
					...
					NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "UpgradeDrill", null, cutterHead, infOreExtractor, 0f);
				}
		...
		else if (name.Equals("storage_icon"))
		{
			if (infOreExtractor.PlayerExtractStoredOre(WorldScript.mLocalPlayer))
			{
				...
				if (!WorldScript.mbIsServer)
				{
					NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "TakeOres", null, null, infOreExtractor, 0f);
				}
				this.dirty = true;
				return true;
			}
		...
		{
			NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "AddPower", null, null, infOreExtractor, this.powerPeriod);
			this.powerPeriod = 0f;
		}
		return false;
	}
	...
	public static void ToggleReport(Player player, InfOreExtractor extractor)
	{
		if (!WorldScript.mbIsServer)
		{
			NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "ToggleReport", null, null, extractor, 0f);
		}
		else
		{
	...
	public static bool DropStoredOre(Player player, InfOreExtractor extractor)
	{
		if (extractor.DropStoredOre())
		{
			if (!WorldScript.mbIsServer)
			{
				NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "DropOres", null, null, extractor, 0f);
			}
			return true;
		}
		return false;
	...
	public override void ButtonDown(string name, SegmentEntity targetEntity)
	{
		...
		if (name.Equals("power_button") && InfExtractorMachineWindow.AddPower(WorldScript.mLocalPlayer, infOreExtractor, Time.deltaTime) && !WorldScript.mbIsServer)
		{
			...
			{
				NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "AddPower", null, null, infOreExtractor, 1f);
				this.powerPeriod -= 1f;
			}
		}
	}
	...
	public override bool RemoveItem(string name, ItemBase originalItem, ItemBase swapitem, SegmentEntity targetEntity)
	{
		...
		if (name.Equals("drill_motor"))
		{
			if (swapitem == null)
			{
				...
				if (!WorldScript.mbIsServer)
				{
					NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "UpgradeMotor", null, null, infOreExtractor, 0f);
				}
				return true;
			}
			if (infOreExtractor.IsValidMotor(swapitem))
			{
				...
				if (!WorldScript.mbIsServer)
				{
					NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "UpgradeMotor", null, swapitem, infOreExtractor, 0f);
				}
				return true;
			}
		}
		else if (name.Equals("cutter_head"))
		{
			if (swapitem == null)
			{
				...
				if (!WorldScript.mbIsServer)
				{
					NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "UpgradeDrill", null, null, infOreExtractor, 0f);
				}
				return true;
			}
			if (infOreExtractor.IsValidCutterHead(swapitem))
			{
				...
				if (!WorldScript.mbIsServer)
				{
					NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "UpgradeDrill", null, swapitem, infOreExtractor, 0f);
				}
				return true;
			}
		}
	...
	public override void HandleItemDrag(string name, ItemBase draggedItem, DragAndDropManager.DragRemoveItem dragDelegate, SegmentEntity targetEntity)
	{
		...
		if (name.Equals("drill_motor"))
		{
			if (infOreExtractor.IsValidMotor(draggedItem))
			{
				...
				if (!WorldScript.mbIsServer)
				{
					NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "UpgradeMotor", null, draggedItem, infOreExtractor, 0f);
				}
			}
		}
		else if (name.Equals("cutter_head") && infOreExtractor.IsValidCutterHead(draggedItem))
		{
			...
			if (dragDelegate(draggedItem, cutterHead))
			{
				...
				if (!WorldScript.mbIsServer)
				{
					NetworkManager.instance.SendInterfaceCommand("InfExtractorMachineWindow", "UpgradeDrill", null, draggedItem, infOreExtractor, 0f);
				}
			}
		}
	}
